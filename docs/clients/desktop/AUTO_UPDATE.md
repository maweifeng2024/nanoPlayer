...
更新：2026-09-11。本文记录当时 v0.1.7 的三平台安装包、更新签名与清单；下文“本轮”、网络失败及 v0.1.6 恢复属于历史记录，不是当前版本结论。历史核验见 [归档成果核对](../../archive/audits/DOCUMENTATION_AUDIT_2026-09-11.md)。
...
新版本：`pnpm release`（默认递增 patch），或传入尚未使用的明确版本号。不要再次发布已存在的 0.1.7。已有标签发布失败：先提交并推送脚本修复，再运行 `pnpm release --resume v0.1.6`。不要移动已经发布的标签。

## 本地推送中断恢复（2026-09-09）

v0.1.7 在本地提交和打标签后，因 GitHub 443 连接超时未能推送。此前 `--resume` 先检查本地领先提交，错误地拒绝这一正常恢复状态。现在先查询远端标签：本地标签存在且远端缺失时，检查工作区干净、标签属于 main、main 可快进，再原子推送 main 与原标签，并等待原始构建。标签已在远端时继续原有云端恢复。网络查询失败不会当成标签缺失；标签冲突或分支分叉会明确报错，保留原标签。

遇到上述历史中断状态可运行 `pnpm release --resume v0.1.7`，不需要重新改版本或删除标签。GitHub 不可达时仍需恢复网络后重试同一命令。

2026-09-11 核验：v0.1.7 已在 GitHub 发布，仓库亦有 `chore(website): publish v0.1.7 downloads` 提交；以上推送中断已不代表当前状态。客户端源码具备更新能力与清单存在，不等于已经完成三平台真实升级验收。
