...
The local command requires authenticated `gh` and Git access. It intentionally stops before changing anything if `main` differs from `origin/main`, the tag already exists, or authentication is missing. `--skip-checks` is available only for diagnosing the automation and should not be used for a public release.
