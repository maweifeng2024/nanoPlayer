...
建议计次规则：时长不低于 30 秒的曲目，在一次播放会话内累计实际播放达到 `min(4 分钟, 曲目时长的 50%)` 时，播放次数增加一次；一次会话最多增加一次。跳过、播放完毕、暂停退出和错误中断都保留事件，从而能计算总聆听时长和最近播放时间。
...
5. 播放次数规则是否采用本文阈值。规则必须在开发前冻结并写入测试，避免统计口径反复变化。
