...
歌曲文件夹多选、播放最多去除时长和响应式修复、首页封面对齐与多图案、歌单副标题精简已实现并通过浏览器及 macOS 实机检查。自动更新客户端和签名产物发布链路已实现；GitHub 私钥 Secret 已获用户授权并保存、验证成功，v0.1.7 已发布三平台安装包、更新签名和更新清单；跨版本安装验证仍未完成。详细行为、验证与外部步骤见 [自动更新说明](AUTO_UPDATE.md)。

## 2026-09-11 文档核对

当前事实与验证边界见 [成果核对](development/DOCUMENTATION_AUDIT_2026-09-11.md)。第 8、11 节保留原始桌面方案，不构成新的开发安排。iOS、安卓、Pad 仅形成[可行性分析](architecture/MOBILE_FEASIBILITY.md)，不纳入本文件的开发里程碑。
