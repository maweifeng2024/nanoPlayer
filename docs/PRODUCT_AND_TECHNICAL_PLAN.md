...
歌曲文件夹多选、播放最多去除时长和响应式修复、首页封面对齐与多图案、歌单副标题精简已实现并通过浏览器及 macOS 实机检查。自动更新客户端和签名产物发布链路已实现；GitHub 私钥 Secret 已获用户授权并保存、验证成功，尚未发布新版或完成跨版本安装验证。详细行为、验证与外部步骤见 [自动更新说明](AUTO_UPDATE.md)。
