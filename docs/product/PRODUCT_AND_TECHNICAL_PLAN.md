...
状态：桌面产品与技术基线（2026-09-11 对齐 v0.1.7；历史里程碑保留为背景）
当前发布平台：macOS universal、Windows x64、Linux x64；移动端仅作独立可行性分析
...
- **网络能力可解释**：在线歌词查询须用户启用；请求字段、来源、匹配置信度和缓存位置可见。桌面自动更新会独立访问 GitHub，不以歌词开关为前提；离线不影响本地播放。
...
左侧一级浏览入口：本地资料库、歌曲、专辑、艺术家、最近添加、播放最多、高评分。最近播放记录仍保留，但不设一级导航。流派继续作为可搜索、可编辑的元数据，不设独立一级页面。扫描问题从“本地资料库”页面进入。
...
flowchart LR
    ...
    DEC --> OS[cpal platform audio backend]
...
### 原始后续范围（历史设想，不是当前发布版本或实施排期）

- 1.1：M3U8 导入导出、智能歌单、迷你播放器、ReplayGain、更多排序/筛选、手动歌词时间偏移。

- Windows 与 Linux 安装包已于 0.1.x 发布；专用原生 SMTC/MPRIS 集成和完整真机格式矩阵仍不能视为完成。原先将整个平台列入 1.2 的假设已过时。
...
1. 产品名为 nanoPlayer，视觉使用深色红黑基线，图标源为 v3。
2. 当前通过 GitHub Release 与官网分发，macOS DMG 未签名/未公证；没有 Mac App Store 已上线证据。
3. 在线歌词默认关闭，用户启用后保存选择；自动更新网络访问另行说明。
4. 当前采用五星评分与高评分视图，没有独立收藏入口。
5. 播放计次口径已于 2026-09-06 改为完整自然结束，回归测试覆盖暂停续播、跳过、拖动进度和循环。

## 11. 原始开发顺序（历史背景，非当前待执行工作）

先做 M0 音频与文件权限样机，再冻结数据 schema 和播放状态机，随后实现资料库纵向切片。不要先投入完整视觉稿或网络歌词；这两部分建立在稳定播放、身份识别和只读扫描之上。

## 2026-09-08 实现状态补充

歌曲文件夹多选、播放最多去除时长和响应式修复、首页封面对齐与多图案、歌单副标题精简已实现并通过浏览器及 macOS 实机检查。自动更新客户端和签名产物发布链路已实现；跨版本安装验证仍未完成。当前客户端状态见 [客户端矩阵](../clients/README.md)，桌面更新行为与历史恢复证据见 [自动更新说明](../clients/desktop/AUTO_UPDATE.md)。

## 2026-09-11 文档核对

历史事实与验证边界见 [归档成果核对](../archive/audits/DOCUMENTATION_AUDIT_2026-09-11.md)。第 8、11 节保留原始桌面方案，不构成新的开发安排。当前 Android 已实现但仍在验收，iOS/iPadOS 尚未实现；跨移动端技术取舍见 [可行性分析](../clients/MOBILE_FEASIBILITY.md)。
