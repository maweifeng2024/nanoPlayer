...
test("home collections match module default order, including ties", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    const tracks = useNanoStore.getState().tracks.map((track, index) => ({
      ...track,
      addedAt: index < 4 ? "2026-09-01" : "2026-09-02",
    }));
  ...
  for (const [kind, name] of [
    ...
    const titles = await home.locator("li .recommendation-copy strong").allTextContents();
    ...
    const ids = await page
      ...
      .evaluateAll((nodes) => nodes.map((n) => Number(n.getAttribute("data-track-id"))));
    const actual = await page.evaluate(async (ids) => {
      const { useNanoStore } = await import("/src/store.ts");
      return ids.map(
        (id) => useNanoStore.getState().tracks.find((track) => track.id === id)?.title,
      );
    }, ids);
...
test("queue centers current track each time it opens and when playback changes", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(async () => {
  ...
  const checkVisible = async () => {
    await expect
      .poll(() =>
        page.locator(".queue-list").evaluate((list) => {
